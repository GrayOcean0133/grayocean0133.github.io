...
## 第六章　决策树（ID3 / C4.5 / CART / 剪枝）

### 6.1 原理
决策树是树状结构的**判别模型**，本质上就是一串**「if-else 判断」**：根节点/非叶节点代表在某个特征上的提问，分支代表该问题的不同答案（特征取值），叶节点代表最终类别。

- **如何预测**：拿一个新样本，从根节点开始，按它在各特征上的取值一路往下走分支，走到哪个叶节点，就预测为该叶节点的类别。就像玩**「二十个问题」**或看一张医生的分诊流程图——每问一个问题就缩小一次范围。
- **如何训练（构建）**：用**贪心算法**自顶向下递归划分。每到一个节点，就从「还没用过的特征」里挑一个**当前最能把样本分纯**的特征来分裂，再对每个分支递归重复，直到满足停止条件（节点内样本已全属同一类、特征用完、或达到预设深度）。贪心意味着**每步只看当下最优、不回溯**，所以决策树不保证全局最优，但简单高效。
- **几何直觉**：每次按一个特征分裂相当于在特征空间里「切一刀」，不断分裂就是把特征空间**递归切成一块块「轴对齐的矩形区域」**，每块区域对应一个叶节点、判一个类别。
- **核心问题**：既然每步都要「挑一个最能分纯的特征」，那**「纯不纯」怎么量化、哪个特征最好**就是决策树算法的全部关键——这正是下面信息论指标（信息增益 / 增益率 / 基尼指数）要回答的问题。
- **突出特点**：可解释性极强（从根到叶的每条路径就是一条人能读懂的规则），不需要标准化，能同时处理数值与类别特征；缺点是**容易过拟合**（树长太深会把噪声也记住），需靠剪枝控制。
...
- **ID3（最早）**：用**信息增益**选特征，思想简单。但有三个硬伤——① 偏爱取值多的特征（见 6.3 的「编号」陷阱）；② 只能处理离散特征，连续值没法用；③ 不剪枝，容易过拟合。
- **C4.5（修补 ID3）**：把信息增益换成**信息增益率**（除以特征自身的熵）来纠正「偏爱多取值特征」的毛病；并支持连续值（按阈值二分）和缺失值，还引入了后剪枝。代价是要算对数，比较慢。
- **CART（另起一套，最常用）**：一律建**二叉树**（每个节点只问「是/否」式的二分问题），分类用**基尼指数**（不含对数、计算更快），回归用**均方差**，因此**既能分类又能回归**——`sklearn.tree` 用的就是 CART。

一句话记忆：**ID3 比增益、C4.5 比增益率、CART 比基尼；前两个是多叉只分类，CART 是二叉能回归**。

### 6.3 信息论基础
决策树选特征，本质是选一个能让「子节点最纯」的特征。「纯不纯」需要一把尺子来量，这把尺子就是**信息熵**——理解了熵，整套指标就都通了。

- **信息熵**（衡量「混乱 / 不确定性」）：$H(D)=-\sum_k p_k\log_2 p_k$。
  - **直觉**：熵度量「猜中一个样本类别平均要花几个 bit」。一个集合越**混乱**（各类别各占一半），越难猜，熵越大；越**纯**（全是同一类），一猜就中，熵越小。
  - **两个边界**：若 $D$ 全属一类，$p=1$，$H=0$（完全确定，最纯）；若两类各占 50%，$H=-0.5\log_2 0.5-0.5\log_2 0.5=1$（最混乱，达到二分类的最大值 1 bit）。
  - 为什么用 $\log_2$：单位是 bit；$-\log_2 p_k$ 是「一个概率为 $p_k$ 的事件发生时带来的信息量」——越罕见的事发生，信息量越大，前面加负号是因为 $\log$ 概率为负。
- **条件熵**（已知特征 A 后还剩多少不确定性）：$H(D|A)=\sum_v \frac{|D_v|}{|D|}H(D_v)$。
  - **直觉**：按特征 A 的取值把 $D$ 分成若干子集 $D_v$，分别算每个子集的熵，再按子集大小**加权平均**。它回答：「**先问了 A 这个问题之后**，数据平均还剩多乱？」
- **信息增益**（ID3 用，越大越好）：$g(D,A)=H(D)-H(D|A)$。
  - **直觉**：= 问 A 之前的混乱 − 问 A 之后的混乱 = **问了 A 这个问题，帮我们消除了多少不确定性**。增益越大，说明这个特征越「一刀见效」，所以 ID3 每步选信息增益最大的特征。
  - **ID3 的偏见（重点）**：信息增益**天然偏爱取值多的特征**。极端例子——给每个样本加一列「编号」（15 个样本 15 个不同值），按编号一分，每个子集只剩 1 个样本、熵全为 0，信息增益直接拉满，于是「编号」被选为根节点。但用编号建的树毫无泛化能力（新样本没有见过的编号）。这就是下一条要修正的问题。
- **信息增益率**（C4.5 用）：$g_R(D,A)=\dfrac{g(D,A)}{H_A(D)}$，其中 $H_A(D)=-\sum_v\frac{|D_v|}{|D|}\log_2\frac{|D_v|}{|D|}$ 是**特征 A 自身的熵**（也叫分裂信息，只看 A 把数据分得多碎、不看类别）。
  - **直觉**：取值越多的特征，把数据分得越碎，$H_A(D)$ 越大。用它去**除**信息增益，相当于给「取值多」的特征**打个折扣**，从而压制 ID3 偏爱编号类特征的毛病。
- **基尼指数**（CART 用，越小越纯）：$Gini(D)=1-\sum_k p_k^2$。
  - **直觉**：从 $D$ 里**有放回地随机抽两个样本，它们类别不同的概率**。越纯，抽到不同类的概率越小，基尼越小（纯集合 $Gini=0$）。
  - **和熵的关系**：基尼是熵的「廉价近似」，两者趋势几乎一样，但**基尼不用算对数**、计算更快，所以 CART 用它替代熵。

### 6.4 剪枝（防过拟合）
树如果一直分到「每个叶子都纯」，往往把训练集的噪声也当规律学了进去——**训练集近乎 100%、测试集却很差**，这就是过拟合。剪枝就是主动「砍掉一些分支」、让树变简单，用一点训练精度换泛化能力。

- **预剪枝（边长边判断）**：在**分裂之前**先评估——如果这次分裂不能提升验证集精度，就**不分**，直接把当前节点设为叶子。
  - 优点：树小、训练快；缺点：太「短视」，可能这一刀看着没用、但再往下两刀才见效，于是过早停止造成**欠拟合**。
- **后剪枝（先长全再回收）**：先让树**充分生长成完整树**，再**自底向上**逐个考察：把某棵子树替换成一个叶节点后，若验证集精度不降反升，就剪掉它。
  - 优点：保留了更多有用分支，**欠拟合风险更小、效果通常更好**；缺点：要先建完整树再逐一回退，**计算量比预剪枝大得多**。
- **一句话**：预剪枝「省力但可能砍过头」，后剪枝「费力但更稳」。`sklearn` 里常用 `max_depth`、`min_samples_leaf`、`ccp_alpha` 等参数来实现剪枝效果。
...
**代码说明**：这是经典的「贷款审批」数据集——根据`年龄/有工作/有自己的房子/信贷情况`判断是否放贷。`calc_ent` 把 6.3 的信息熵公式 $H(D)=-\sum p_k\log_2 p_k$ 直接翻译成代码：统计每个类别的占比再代入公式。这里 15 个样本中 9 个「是」、6 个「否」，算得 $H(D)\approx0.971$（接近 1，说明放/不放贷比较「五五开」、很不确定）。`cond_ent` 则实现条件熵：按某个特征的取值把数据分组，对每组算熵再按组大小加权求和。这两个函数是下面算信息增益的「零件」。
...
**代码说明**：前半段手写 `gini` 验证基尼公式——`[0,0,1,1]` 两类各半，$Gini=1-(0.5^2+0.5^2)=0.5$，达到二分类基尼的最大值（最不纯）。后半段调 `sklearn` 的 `DecisionTreeClassifier`：`criterion='gini'` 即用基尼指数选特征（CART 的做法），`max_depth=3` 是**预剪枝**——人为限制树最多 3 层，防止过拟合。`plot_tree` 把训练好的树画出来，每个节点会显示分裂条件、`gini` 值、样本数和类别分布，能直观看到「越往下基尼越小、越纯」。
...
**代码说明**：决策树也能做**回归**——CART 回归树用「均方差」选分裂点，叶节点输出落入该区域样本的**均值**，所以拟合曲线是**阶梯状**（每段对应一个叶子区域的常数预测）。这张对比图是「过拟合」最直观的演示：`max_depth=5` 的树太深，把人为加的噪声点也当规律学了进去（阶梯过细、上下乱跳），而 `max_depth=2` 更平滑、更接近真实的 $\sin$ 曲线。这正说明**限制深度 / 剪枝**的必要性——树不是越深越好。
